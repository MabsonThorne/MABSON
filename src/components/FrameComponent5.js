...
const FrameComponent5 = ({ className = "" }) => {
  ...
  return (
    <div className={`w-full flex flex-row items-center justify-between py-4 px-4 box-border text-left text-29xl text-red font-small-text ${className}`}>
      ...
      <div className="flex flex-row items-center justify-center gap-6">
        <div className="flex items-center">
          <h1 className="m-0 text-xl leading-6 font-medium cursor-pointer" onClick={() => navigate('/')}>
            首页
          </h1>
        </div>
        <div className="flex items-center">
          <h1 className="m-0 text-xl leading-6 font-medium cursor-pointer" onClick={() => navigate('/1')}>
            采购端
          </h1>
        </div>
        <div className="flex items-center">
          <h1 className="m-0 text-xl leading-6 font-medium cursor-pointer" onClick={() => navigate('/2')}>
            需求端
          </h1>
        </div>
      </div>
      <div className="flex flex-row items-center justify-end gap-5 text-base">
        ...
        <div className="flex items-center justify-start pt-1.5">
          {userProfile && (
            <>
              <img className="h-10 w-10 rounded-full object-cover" src={userProfile.avatar_file || "/path/to/default-avatar.png"} alt="User Avatar" />
              <div>{userProfile.username}</div>
              <div>{userProfile.email}</div>
            </>
          )}
          <input type="file" accept="image/*" onChange={onAvatarChange} />
          <textarea value={bio} onChange={onBioChange} placeholder="简介"></textarea>
